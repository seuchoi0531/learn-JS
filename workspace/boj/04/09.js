...
for (let i = 1; i <= m; i++) {
  let tmp = [];
  for (let j = parseInt(input[i].split(" ")[0]) - 1; j < parseInt(input[i].split(" ")[1]); j++) tmp.push(arr[j]);
  tmp.reverse();
  for (let j = parseInt(input[i].split(" ")[0]) - 1; j < parseInt(input[i].split(" ")[1]); j++) arr[j] = tmp[j - parseInt(input[i].split(" ")[0]) + 1];
}
let rst = "";
for (i of arr) rst += i + " ";
console.log(rst);
