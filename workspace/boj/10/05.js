...
while (q--) {
  ...
  let max = -10000;
  for (let i = 1; i <= n; i++) {
    max = max < parseInt(input[i].trim().split(" ")[q]) ? parseInt(input[i].trim().split(" ")[q]) : max;
    min = min > parseInt(input[i].trim().split(" ")[q]) ? parseInt(input[i].trim().split(" ")[q]) : min;
  }
  s *= max - min;
}
console.log(s);
